...
export function CheatSheetForm({ categories, onSuccess }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Filter documents that have both label and file
    ...
    try {
      ...
      if (validDocs.length > 0) {
        ...
        // Use the first document's file as the resource's file_url for backward compat
        filePath = uploadedDocs[0].file_url
      } else if (documents[0]?.file) {
        // Single file without label — legacy behavior
        const formData = new FormData()
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        ...
        <Input
          ...
          placeholder="e.g. Pricing Strategy Cheat Sheet"
          required
        />
      </div>
      ...
      <div>
        <label className="text-sm font-medium mb-1.5 block">Documents</label>
      ...
      <Button type="submit" disabled={loading} className="w-full">
        ...
        {loading ? "Creating..." : "Create Cheat Sheet"}
      </Button>
    </form>
  )
}
