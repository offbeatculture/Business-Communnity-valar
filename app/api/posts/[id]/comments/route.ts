...
export async function POST(
  ...
  try {
    ...
    // Fetch the author's profile separately (no direct FK between comments and profiles)
    ...
    // Increment comment_count via admin client
    ...
    if (post) {
      ...
      // Award GP only when commenting on someone else's post
      if (post.user_id !== user.id) {
        const commentGP = parsed.data.content.length >= COMMENT_SUBSTANTIVE_LENGTH
          ? GP_VALUES.comment_substantive
          : GP_VALUES.comment_short
        awardPoints(user.id, "comment", commentGP, comment.id).catch(() => {})
        awardPoints(post.user_id, "comment_received", GP_VALUES.comment_received, comment.id).catch(() => {})
      }
    }
